...
1. **Diagnóstico (< 1 min)**
   ```bash
   # Verificar estado en Supabase dashboard:
   # https://app.supabase.com → proyecto securecode-mvp → Database
   # O verificar logs de Cloud Run:
   gcloud run logs read securecode --region us-central1 --limit 50
   # Resultado: identificar si es error de BD o de API
   ```

2. **Activación Backup (< 2 min)**
   ```bash
   # Supabase guarda backups automáticos diarios
   # Restaurar desde: Supabase Dashboard → Settings → Database → Backups
   # Seleccionar el snapshot más reciente → Restore
   # (También disponible por CLI si se tiene supabase CLI instalado)
   supabase db restore --backup-id <latest_backup_id>
   ```

3. **Validación (< 1 min)**
   ```bash
   # Reconectar y verificar integridad
   psql $DATABASE_URL -c "SELECT COUNT(*) FROM evaluations"
   # Debe retornar número > 0
   ```

4. **Reinicio API (< 1 min)**
   ```bash
   # Cloud Run redeploy desde la última imagen en Artifact Registry:
...
**RPO:** 24 horas (backup diario automático Supabase)
...
2. **Rotación Token**
   ...
   # Actualizar en Cloud Run (redeploy con nuevo token):
   gcloud run deploy securecode \
     --region us-central1 \
     --platform managed \
     --update-env-vars GITHUB_API_TOKEN="ghp_new_token_here"
   ```
